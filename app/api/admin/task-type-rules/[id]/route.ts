...
export async function PATCH(request: Request, ctx: Ctx) {
  try {
    debugAdminRouteIncoming(request, "task-type-rules-id");
    const gate = await requireAdminRequest(request, "task-type-rules-id");
    debugAdminRouteGateOutcome("task-type-rules-id", gate);
    ...
    if (error) {
      console.error("[api/admin/task-type-rules PATCH] SUPABASE_QUERY_ERROR", error);
      return NextResponse.json({ error: error.message }, { status: 400 });
    }
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[api/admin/task-type-rules PATCH] UNEXPECTED", err);
...
export async function DELETE(request: Request, ctx: Ctx) {
  try {
    ...
    if (error) {
      console.error("[api/admin/task-type-rules DELETE] SUPABASE_QUERY_ERROR", error);
      return NextResponse.json({ error: error.message }, { status: 400 });
    }
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[api/admin/task-type-rules DELETE] UNEXPECTED", err);
    return NextResponse.json(
      {
        error: "UNEXPECTED",
        message: err instanceof Error ? err.message : String(err),
